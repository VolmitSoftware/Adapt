...
public class RegistryUtil {
  ...
  public static <T> Optional<T> findByField(@NonNull Class<T> typeClass, @NonNull NamespacedKey... keys) {
    java.util.Map<org.bukkit.NamespacedKey, org.bukkit.Keyed> values = KEYED_REGISTRY.computeIfAbsent(typeClass, RegistryUtil::getKeyedValues);
    for (NamespacedKey key : keys) {
      org.bukkit.Keyed value = values.get(key);
      if (value != null)
        return Optional.of((T) value);
    }
    return Optional.empty();
  ...
  public static <T> Optional<T> findByEnum(@NonNull Class<T> typeClass, @NonNull NamespacedKey... keys) {
    java.util.Map<org.bukkit.NamespacedKey, java.lang.Object> values = ENUM_REGISTRY.computeIfAbsent(typeClass, RegistryUtil::getEnumValues);
    for (NamespacedKey key : keys) {
      java.lang.Object value = values.get(key);
      if (value != null)
        return Optional.of((T) value);
    }
    return Optional.empty();
