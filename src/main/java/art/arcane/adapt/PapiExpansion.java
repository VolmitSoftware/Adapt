...
public class PapiExpansion extends PlaceholderExpansion {
  ...
  private String resolvePlayerAttribute(PlayerData playerData, String rawAttr) {
    ...
    return switch (attr) {
      ...
      case "adaptations", "adaptationcount" ->
          String.valueOf(countAdaptations());
      case "learnedadaptations" ->
          String.valueOf(countLearnedAdaptations(playerData));
      default -> "0";
    };
  }
  ...
  private String resolveSkillAttribute(PlayerData playerData, Skill<?> skill, PlayerSkillLine line, String rawAttr) {
    ...
    return switch (attr) {
      ...
      case "knowledge" ->
          String.valueOf(line == null ? 0 : line.getKnowledge());
      ...
      case "absolute_level", "absolutelevel" ->
          format4(line == null ? 0 : line.getAbsoluteLevel());
      case "progress" -> format4(line == null ? 0 : line.getLevelProgress());
      case "progress_percent", "progresspercent" ->
          format2((line == null ? 0 : line.getLevelProgress()) * 100D);
      case "xp_to_next", "xptonext", "xp_needed", "xpneeded" ->
          format2(line == null ? 0 : XP.getXpUntilLevelUp(line.getXp()));
      case "current_level_xp", "currentlevelxp" ->
          format2(XP.getXpForLevel(currentLevel));
      case "next_level_xp", "nextlevelxp" ->
          format2(XP.getXpForLevel(currentLevel + 1));
      case "adaptation_count", "adaptationcount" ->
          String.valueOf(skill.getAdaptations().size());
      case "learned_adaptations", "learnedadaptations" ->
          String.valueOf(learnedAdaptations);
      case "can_use" -> String.valueOf(currentLevel > 0);
      case "line" -> skill.getName();
      case "maxlevel", "max_level" ->
          String.valueOf(AdaptConfig.get().experienceMaxLevel);
      default -> "0";
    };
  }
  ...
  private String resolveAdaptationAttribute(PlayerData playerData, Adaptation<?> adaptation, String rawAttr) {
    ...
    return switch (attr) {
      ...
      case "can_use" ->
          String.valueOf(currentLevel > 0 && adaptation.isEnabled() && adaptation.getSkill().isEnabled());
      case "cost_next", "costnext", "knowledge_next", "knowledgenext" ->
          String.valueOf(currentLevel >= adaptation.getMaxLevel() ? 0 : adaptation.getCostFor(nextLevel, currentLevel));
      case "power_next", "powernext" ->
          String.valueOf(currentLevel >= adaptation.getMaxLevel() ? 0 : Math.max(0, adaptation.getPowerCostFor(nextLevel, currentLevel)));
      case "refund_next", "refundnext" ->
          String.valueOf(currentLevel <= 0 ? 0 : adaptation.getRefundCostFor(Math.max(0, currentLevel - 1), currentLevel));
