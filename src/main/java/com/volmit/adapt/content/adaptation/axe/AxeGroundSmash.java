...
public class AxeGroundSmash extends SimpleAdaptation<AxeGroundSmash.Config> {
    ...
    public void addStats(int level, Element v) {
        ...
        v.addLore(Localizer.dLocalize(
                "axe.ground_smash.lore",
                Form.f(getFalloffDamage(f), 1),
                Form.f(getDamage(f), 1),
                Form.f(getRadius(f), 1),
                Form.pc(getForce(f), 0),
                Form.duration(getCooldownTime(f) * 50D, 1)));
    }
