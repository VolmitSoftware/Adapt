        ...
        if (!adaptPlayer.getData().getSkillLines().isEmpty()) {
            for (PlayerSkillLine i : adaptPlayer.getData().getSkillLines().sortV()) {
                ...
                w.setElement(pos, row, new UIElement("skill-" + sk.getName())
                        ...
                        .addLore("")
                        .addLore(Localizer.dLocalize("snippets.gui.knowledge", i.getKnowledge()))
                        .addLore(Localizer.dLocalize("snippets.gui.power_used", adaptationLevel))
            ...
            if (AdaptConfig.get().isUnlearnAllButton()) {
                ...
                w.setElement(unlearnAllPos, unlearnAllRow, new UIElement("unlearn-all")
                        ...
                        .setModel(CustomModel.get(Material.BARRIER, "snippets", "gui", "unlearn_all"))
                        .setName(Localizer.dLocalize("snippets.gui.unlearn_all"))
                        .addLore(AdaptConfig.get().isHardcoreNoRefunds() ? Localizer.dLocalize("snippets.adapt_menu.no_refunds") : null)
                        .onLeftClick((e) -> {
                            ...
                            if (AdaptConfig.get().getLearnUnlearnButtonDelayTicks() != 0) {
                                player.sendTitle(" ", Localizer.dLocalize("snippets.gui.unlearned_all"), 1, 5, 11);
                            }
                            J.s(() -> open(player), AdaptConfig.get().getLearnUnlearnButtonDelayTicks());
                        }));
            }

            w.setTitle(Localizer.dLocalize("snippets.gui.level", (int) XP.getLevelForXp(adaptPlayer.getData().getMasterXp())) + " " + Localizer.dLocalize("snippets.gui.power_used_ratio", adaptPlayer.getData().getUsedPower(), adaptPlayer.getData().getMaxPower()));
            w.open();
